...
|---------|------------------------|------------------------|
| 新建/修改 原始数据项目 | ✅ 显示（只读文本框） | ✅ 显示（可编辑） |
| 新建/修改 结果项目 | ✅ 显示（只读文本框） | ✅ 显示（可编辑） |
| 文件导入 原始数据 | ❌ 不显示 | ✅ 显示前 5 个（按 field_seq 排序），可编辑 |
| 文件导入 结果项目 | ❌ 不显示 | ✅ 显示前 3 个（按 field_seq 排序），可编辑 |

**说明**：
- `field_readonly=1` 的字段在项目详情页面以只读方式显示，在文件导入页面不显示
- `field_readonly=0` 的字段在项目新建/修改页面全部显示
- **文件导入页面**：取 `field_readonly=0` 的字段，**排除前端已硬编码的前2个字段（项目编号、项目名称）**后，原始数据项目取排序前 3-7 号（共 5 个），结果数据项目取排序前 3-5 号（共 3 个）
- **选择已有项目导入时，字段可编辑**：如果用户修改了字段值（如 raw_type、raw_species），文件将存储到新路径（追加模式），原路径文件保持不变
...
('file_size', '文件大小', 'text', 'file', 0, 2, '2', 0),
('file_type', '文件类型', 'text', 'file', 0, 3, '2', 0),
('file_project_type', '项目类型', 'text', 'file', 1, 4, '2', 0),
('file_project_id', '项目编号', 'text', 'file', 1, 5, '2', 0),
('imported_at', '导入时间', 'text', 'file', 0, 6, '2', 0);
```
