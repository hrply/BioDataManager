...
## 八、文件导入 overwrite 参数规则

### 8.1 同名文件检测与处理

当导入文件时，系统会检测同一项目同一路径下是否存在同名文件：

```python
def add_file_record(self, project_type: str, project_id: str, file_path: Path, 
                   metadata: Dict = None, ref_project_id: str = None, 
                   overwrite: bool = False) -> Dict:
    # 检测重复文件（同一项目、同一路径下同名文件才算真正重复）
    ...
    if existing:
        if overwrite:
            ...
            return {
                'success': False,
                'message': f'文件 "{file_name}" 已存在于该路径下，请检查是否需要覆盖',
                'is_duplicate': True,
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `overwrite` | boolean | 是否覆盖已存在的同名文件，默认 false |
...
### 8.3 raw_tissue 中文验证规则

```python
raw_tissue = self._validate_comma_separated(raw_tissue_input, '组织来源 (raw_tissue)', allow_chinese=True)
```

**验证规则：**
- `allow_chinese=True`：允许中文逗号、中文字符（用于组织名称如"肝"、"心"等）
- 支持的中文逗号：`，`（U+FF0C）
- 存储时统一转换为英文逗号分隔
...
**主要更新 (v2.7)**:
- 新增文件导入 overwrite 参数支持同名文件处理
- add_file_record 新增 overwrite 模式和删除已有记录逻辑
- raw_tissue 验证新增 `allow_chinese=True` 支持中文组织名称
- select 字段验证规则：存储 value，前端显示 label
...
| 字段名 | 类型 | 说明 | 长度 |
|--------|------|------|------|
| file_MD5 | CHAR(32) | 文件 MD5 哈希值 | 32 位十六进制 |
| file_SHA256 | CHAR(64) | 文件 SHA256 哈希值 | 64 位十六进制 |

**添加位置**：位于 `file_project_ref_id` 字段之后
...
| 字段 | 存储值类型 | 存储规则 | 示例 |
|------|-----------|----------|------|
| file_MD5 | 十六进制字符串 | 32 位 MD5 哈希值 | "2592ca784579349ec3caf1d063d5d9ad" |
| file_SHA256 | 十六进制字符串 | 64 位 SHA256 哈希值 | "e9f92fe99d7bdc6444de7317c63bcb2b8dd24780e2035727f93a969652d1b9f5" |

**功能说明**：

1. **文件完整性校验**：
   - MD5：快速验证文件是否被篡改（速度快，但存在碰撞风险）
   - SHA256：高强度验证文件完整性（更安全，但计算较慢）

2. **异步计算**：
   - 使用 TaskManager 异步任务管理器
   - 支持大文件（GB 级别）计算
   - 后台线程执行，不阻塞 API 请求

3. **存储格式**：
   - MD5：固定 32 位十六进制小写
   - SHA256：固定 64 位十六进制小写
   - 示例：`0-9a-f` 字符
...
5. **API 接口**：
   - `POST /api/files/hash/calculate` - 创建异步计算任务
   - `GET /api/files/hash/status/<task_id>` - 查询任务状态
...
|--------|---------|------|
| 小文件测试 | 10MB | ✅ 计算成功 |
| 中等文件测试 | 20MB（多文件） | ✅ 计算成功 |
| 大文件测试 | 47GB | ✅ 计算成功（2-3分钟） |
| API 响应 | - | ✅ 立即返回任务 ID |
| 超时测试 | 47GB | ✅ 无超时问题 |
