...
| 完整路径 | file_path | file_name | file_type |
|---------|-----------|-----------|-----------|
| /bio/mRseq/Hs/Li/sample1.fastq.gz | mRseq/Hs/Li | sample1.fastq.gz | gz |
| /bio/rawdata/mRseq/Hs/Li/RAW_XXX/sample.fastq | rawdata/mRseq/Hs/Li | sample.fastq | fastq |
...
**数据库版本**: MySQL 8.x  
**主要更新**:
- 更新 file_record 表字段规范（file_path、file_name、file_type）
- file_path 改为存储相对于 /bio 的路径（不包含文件名）
- file_name 存储完整文件名（包含扩展名）
- file_type 存储文件扩展名（小写）
- field_type 简化为 text/textarea/select 三个值
- 移除 multi_select 类型，改为下拉框行为模式说明
- 新建项目/编辑项目下拉框使用 multi_select 模式
- 文件导入下拉框使用 single_select 模式
