...
CREATE TABLE field_config (
    ...
    field_type      ENUM('text', 'textarea', 'select', 'multi_select', 'link', 'tags') NOT NULL DEFAULT 'text' COMMENT '字段类型',
...
| `select` | 单选下拉 | 显示 value 对应的 label |
| `multi_select` | 多选下拉 | 显示 value 对应的 label |
| `link` | 链接字段 | 超链接可点击 |
...
| raw_type | 字符串 | select_options.option_value | "mRNAseq", "蛋白组" |
| raw_species | 字符串 | select_options.option_value | "Homo sapiens" |
| raw_tissue | 字符串 | 多选逗号分隔 | "Lung,Tumor" |
...
| results_type | 字符串 | 结果类型标识 | "DEA", "Marker" |
| results_raw | 字符串 | 关联原始项目编号 | "RAW_A1B2C3D4" |
...
| 结果数据 (result) | `分析类型-[关联项目编号]` | "差异分析-RAW_A1B2C3D4" |
