...
- **递归子目录文件导入修复**：修复从递归扫描结果中导入子目录文件时，只导入顶层文件夹文件的问题。现在前端会传递文件的完整相对路径 (`relative_path`)，后端根据相对路径构建正确的源文件路径，支持从多层嵌套子目录导入文件
...
**修复说明（2026-03-20）**：
- **问题**：从递归扫描结果中导入子目录文件时，只导入了顶层文件夹中的文件，子目录文件导入失败（显示"成功导入 0/N 个文件"）
- **原因**：前端只传递了顶层文件夹名（如 `10.1038_s41467-024-45665-6`）和文件名，但文件实际在子目录中（如 `10.1038_s41467-024-45665-6/cytof/pbl_cytof/file.fcs`），导致后端无法找到源文件
- **修复**：
  1. 前端 `renderFileSelection()`：checkbox 的 `value` 改为存储 JSON 格式 `{name: '文件名', relative_path: '相对路径'}`
  2. 前端 `confirmImport()`：解析 JSON 提取 `relative_path`，并传递完整的文件对象列表给后端
  3. 后端 `_import_raw_files()`：优先使用 `relative_path` 构建完整的源文件路径（`/bio/downloads/{relative_path}`）
- **修复文件**：`app/templates/files.html`、`app/backend.py`
