...
(async () => {
    ...
    try {
        ...
        if (folderCount > 0) {
            ...
            console.log(`5. 点击第 ${targetIndex + 1} 个文件夹的导入按钮...`);
            await importBtns[targetIndex].click();
            await page.waitForTimeout(1000);
            
            // 6. 选择"导入至已有项目"
            console.log('6. 选择"导入至已有项目"...');
            ...
            console.log(`7. 项目选项: ${JSON.stringify(projectOptions)}`);
            ...
            if (projectOptions.length > 1) {
                ...
                // 9. 清空元数据字段（测试不修改任何字段的情况）
                console.log('9. 测试场景：不清除任何字段，直接导入...');
                ...
                // 13. 测试结果
                const importSuccess = consoleMessages.some(m => 
                    m.text.includes('导入成功') || 
                    (m.text.includes('API response') && m.text.includes('"imported"'))
                );
                const importError = errors.some(e => e.includes('必填字段不能为空'));
                
                console.log('\n=== 测试结果 ===');
                console.log('导入API调用成功:', !errors.some(e => e.includes('page.click')));
                console.log('没有必填字段错误:', !importError);
